...
export default function Programmers() {
  ...
  const [idProgrammer, setIdProgrammer] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers[idProgrammer - 1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idProgrammer ? 'gold': 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setIdProgrammer(dev.id) }>Feature</button>
            </div>
          )
        }
      </div>
